...
pub(crate) fn game_loop(mut rl: RaylibHandle, mut thread: RaylibThread, settings: Settings) {
    ...
    while !rl.window_should_close() {
        ...
        draw_frame(&mut rl, &thread, &settings, &mut player_1, &mut player_2, &level_data);
    }
}

fn draw_frame(rl: &mut RaylibHandle, thread: &RaylibThread, settings: &Settings, player_1: &mut PlayerSimple, player_2: &mut PlayerSimple, level_data: &Vec<Rectangle>) {
